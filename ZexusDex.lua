...
local function main()
	...
	Explorer.ShowRightClick = function()
		...
		if presentClasses["BasePart"] or presentClasses["Model"] then
			...
			context:AddRegistered("TELEPORT_OBJECT_TO_ME")
			context:AddRegistered("VIEW_OBJECT")
		end

		context:AddDivider()
		if presentClasses["BasePart"] or presentClasses["Model"] or presentClasses["Union"] or presentClasses["Mesh"] or presentClasses["Decal"] then
		context:AddRegistered("DELETED_TEXTURES")
		end

		context:AddDivider()
		if presentClasses["Animation"] then
		context:AddRegistered("PLAY_ANIMATION_IN_PLR")
		context:AddRegistered("PLAY_ANIMATION_AT_OBJECT")
		end
		...
		if presentClasses["RemoteEvent"] or presentClasses["RemoteFunction"] or presentClasses["BindableEvent"] or presentClasses["BindableFunction"] then context:AddRegistered("CALL_REMOTE") end
	...
	Explorer.InitRightClick = function()
		...
		context:Register("DELETED_TEXTURES", {
			Name = "Deleted Textures", 
			Icon = "rbxassetid://18920148893", 
			OnClick = function()
				local sList = selection.List
				local isa = game.IsA
							...
							for _, part in pairs(object:GetDescendants()) do
								if isa(part, "BasePart") then
									for _, texture in pairs(part:GetChildren()) do
										if isa(texture, "Texture") or isa(texture, "Decal") then
											texture:Destroy()
										end
									end
								end
							end
						else -- Если объект - BasePart, удаляем текстуры прямо из него
		...
		context:Register("TELEPORT_OBJECT_TO_ME", {
			Name = "Teleport Object To Me", 
			...
			OnClick = function()
				...
				for i = 1, #sList do
					...
					if isa(node.Obj, "BasePart") then
						node.Obj.CFrame = hrp.CFrame + Settings.Explorer.TeleportToOffset
						break
					elseif isa(node.Obj, "Model") then
						if node.Obj.PrimaryPart then
							node.Obj:SetPrimaryPartCFrame(hrp.CFrame + Settings.Explorer.TeleportToOffset)
							break
						else
							local part = node.Obj:FindFirstChildWhichIsA("BasePart", true)
							if part and nodes[part] then
								nodes[part].Obj.CFrame = hrp.CFrame + Settings.Explorer.TeleportToOffset
							end
						end
					end
				end
		...
		context:Register("COPY_PATH", {
			...
			OnClick = function()
				local sList = selection.List
				if #sList == 1 then
					env.setclipboard("game." .. clth(Explorer.GetInstancePath(sList[1].Obj)))
				elseif #sList > 1 then
					local resList = {"{"}
					local count = 2
					for i = 1, #sList do
					...
					resList[count] = "}"
					env.setclipboard(table.concat("game." .. resList, "\n"))
				end
			end
		})
					...
					if obj and obj:IsA("ModuleScript") then
						local success, result = pcall(function()
							...
							if type(module) == "function" then
								return module()  -- Вызов функции в ModuleScript
							else
								warn("ModuleScript does not contain a function")
							end
						end)
						if not success then
							warn("Error calling ModuleScript function: ", result)
						else
							print("ModuleScript result: ", result)
						end
					elseif obj and obj:IsA("Script") then
						local success, result = pcall(function()
							local func, err = loadstring(obj.Source)
							if func then
								return func()
							else
								error(err)
						...
						warn("Selected object is not a ModuleScript or Script")
					end
				elseif #sList > 1 then
		...
		context:Register("CALL_REMOTE", {
			Name = "Call Remote",
			IconMap = Explorer.ClassIcons,
			Icon = 66,
			OnClick = function()
				local sList = selection.List
				local unpackArgs = function(args)
				...
				local callObject = function(obj)
					local args = obj:GetAttribute("Arguments") or {}
					...
					elseif obj:IsA("RemoteFunction") then
						local success, result = pcall(function()
							return obj:InvokeServer(args)
						end)
						if not success then
							warn("Error calling RemoteFunction: ", result)
						else
					...
					elseif obj:IsA("BindableFunction") then
						...
						if not success then
							warn("Error calling BindableFunction: ", result)
						else
							print("BindableFunction result: ", result)
						end
					else
						warn("Selected object is not a RemoteEvent, RemoteFunction, BindableEvent, or BindableFunction")
					end
				end
...
context:Register("PLAY_ANIMATION_IN_PLR", {
    Name = "Play Animation in My player",
    IconMap = Explorer.ClassIcons,
    Icon = 29,
    OnClick = function()
        local sList = selection.List
        if #sList == 1 then
            ...
            if obj and obj:IsA("Animation") then
                ...
                if humanoid then
                    ...
                    if not success then
                        warn("Failed to play animation: " .. tostring(err))
                    end
                else
                    warn("Humanoid not found in character.")
                end
            else
                warn("Selected object is not an Animation.")
            end
        else
            warn("Please select a single Animation object.")
        end
    end
})


context:Register("PLAY_ANIMATION_AT_OBJECT", {
    Name = "Play Animation at Object",
    IconMap = Explorer.ClassIcons,
    Icon = 29,
    OnClick = function()
        local sList = selection.List
        if #sList == 1 then
            ...
            if obj and obj:IsA("Animation") then
                ...
                local humanoid = parent:FindFirstChildOfClass("Humanoid") or parent:FindFirstChildWhichIsA("Humanoid", true)
                local animator = parent:FindFirstChildOfClass("Animator") or parent:FindFirstChildWhichIsA("Animator", true)

                if humanoid or animator then
                    ...
                    warn("No Humanoid or Animator found in the object or its descendants.")
                end
            else
                warn("Selected object is not an Animation.")
            end
		...
		context:Register("GET_REFERENCES", {
			...
			OnClick = function()
				...
				for _, item in ipairs(sList) do
					local obj = item.Obj
					if obj then
						local references = getreferences(obj)  -- Предполагаемая функция, возвращающая все ссылки на объект в коде
						print("References for " .. obj:GetFullName() .. ":")
		...
		context:Register("SAVE_INST", {
			...
			OnClick = function()
				...
				for _, item in ipairs(sList) do
					local obj = item.Obj
					if obj then
						local data = game:GetService("HttpService"):JSONEncode(obj:GetAttributes())
						local filename = obj:GetFullName() .. ".json"
						env.writefile(filename, data)
						setclipboard("Instance saved to " .. filename)
					end
				end
			end
		})
		...
		context:Register("VIEW_CONNECTIONS", {
			...
			OnClick = function()
				...
				for _, item in ipairs(sList) do
					local obj = item.Obj
					if obj then
						local connections = getconnections(obj)  -- Предполагаемая функция для получения всех подключений объекта
						print("Connections for " .. obj:GetFullName() .. ":")
			...
			OnClick = function()
				...
				for _, item in ipairs(sList) do
					...
					if obj then
						...
						local apiPage = "https://create.roblox.com/docs/reference/engine/classes/" .. className
						setclipboard(className)
						env.openURL(apiPage)  -- Открытие страницы API в браузере
					end
				end
			end
		})
		...
		context:Register("FIRE_PROXIMITYPROMPT", {
			...
			OnClick = function()
				...
				for _, v in ipairs(selection.List) do
					if v.Obj:IsA("ProximityPrompt") then
						local prompt = v.Obj
						
						-- Имитация удержания действия ProximityPrompt
						prompt:InputHoldBegin()
						
						local holdDuration = prompt.HoldDuration or 0.1 -- Длительность удержания, по умолчанию 0.5 секунд
						wait(holdDuration + 0.1) -- Задержка с запасом для надежности
		
						prompt:InputHoldEnd()
						
						print("ProximityPrompt activated for: ", prompt.Parent:GetFullName())
					else
						warn("Selected object is not a ProximityPrompt")
					end
				end
			end
		})
		...
		context:Register("VIEW_SCRIPT", {
			...
			OnClick = function()
				local sList = selection.List
				if #sList == 0 then
					warn("No script selected. Please select a script to view.")
					return
				end
				...
				for _, item in ipairs(sList) do
					...
					if scr and (scr:IsA("Script") or scr:IsA("LocalScript") or scr:IsA("ModuleScript")) then
						...
						warn("Selected object is not a valid script.")
					end
				end
			end
		})
...
local function main()
	...
	ScriptViewer.ViewScript = function(scr)
		if scr and (scr:IsA("Script") or scr:IsA("LocalScript") or scr:IsA("ModuleScript")) then
			...
			if success and source then
				PreviousScr = scr
				codeFrame:SetText(source:gsub("\0", "\\0")) -- Исправление проблемы с символом \0
			else
				warn("Failed to decompile the script. Showing default message.")
				codeFrame:SetText("-- DEX - Source failed to decompile - Error Your Executor UNC Bad")
				PreviousScr = nil
			end
	
			window:Show()
		else
			warn("Selected object is not a supported script type.")
		end
	end
	...
	ScriptViewer.Init = function()
		...
		save.MouseButton1Click:Connect(function()
			...
			print("Source code saved to file: " .. filename)
		end)
	
		-- Кнопка дампа функций
		...
		dumpbtn.MouseButton1Click:Connect(function()
			if PreviousScr ~= nil then
				pcall(function()
					-- Dumping functions (thanks to King.Kevin)
					...
					print("Functions dumped successfully.")
				end)
			else
				warn("No script selected for function dumping.")
			end
		end)
end
...
return {Classes = classes, Enums = enums, PropertyOrders = propertyOrders}
	...
	Main.CreateIntro = function(initStatus) -- TODO: Must theme and show errors
		local gui = create({
			...
			{5,"TextLabel",{BackgroundColor3=Color3.new(1,1,1),BackgroundTransparency=1,Font=4,Name="Title",Parent={3},Position=UDim2.new(0,-190,0,15),Size=UDim2.new(0,100,0,50),Text="Dex OP Edition",TextColor3=Color3.new(1,1,1),TextSize=30,TextTransparency=1,}},
			{6,"TextLabel",{BackgroundColor3=Color3.new(1,1,1),BackgroundTransparency=1,Font=3,Name="Desc",Parent={3},Position=UDim2.new(0,-230,0,60),Size=UDim2.new(0,180,0,25),Text="Client package and GST pack",TextColor3=Color3.new(1,1,1),TextSize=14,TextTransparency=1,}},
			...
			{11,"TextLabel",{BackgroundColor3=Color3.new(1,1,1),BackgroundTransparency=1,Font=3,Name="Creator",Parent={2},Position=UDim2.new(1,-110,1,-20),Size=UDim2.new(0,105,0,20),Text="Developed Moon and Zexus",TextColor3=Color3.new(1,1,1),TextSize=14,TextXAlignment=1,}},
	...
	Main.CreateMainGui = function()
		...
		Main.CreateApp({Name = "Script Viewer (Not Working)", IconMap = Main.LargeIcons, Icon = "Script_Viewer", Window = ScriptViewer.Window})
		...
		local cptsOnMouseClickF = nil
		Main.CreateApp({
		Name = "Click to Deleted", 
		Icon = "rbxassetid://18977154158", 
		OnClick = function(callback)
			if callback then
				local moused = Main.Mouse
				cptsOnMouseClickF = moused.Button1Down:Connect(function()
					pcall(function()
						local object = moused.Target
						if nodes[object] then
							object:Destroy()
						end
					end)
				end)
			else if cptsOnMouseClickF ~= nil then cptsOnMouseClickF:Disconnect() cptsOnMouseClickF = nil end end
		end})
		...
		local rotationStep = 25
		Main.CreateApp({
			Name = "Drag Obj", 
			Icon = "rbxassetid://18977061398", 
			OnClick = function(callback)
				if callback then
					local mouse = Main.Mouse
					...
					mouse.Button1Down:Connect(function()
						local target = mouse.Target
							...
							local offset = selectedObject.Position - mouse.Hit.Position
							dragConnection = mouse.Move:Connect(function()
								if dragging and selectedObject then
									selectedObject.Position = mouse.Hit.Position + offset
								end
							end)
					...
					userInput.InputBegan:Connect(function(input, processed)
						if not processed and selectedObject then
							if userInput:IsKeyDown(Enum.KeyCode.LeftControl) or userInput:IsKeyDown(Enum.KeyCode.RightControl) then
								if input.KeyCode == Enum.KeyCode.R then
									selectedObject.CFrame = selectedObject.CFrame * CFrame.Angles(0, math.rad(rotationStep), 0)
								elseif input.KeyCode == Enum.KeyCode.T then
									selectedObject.CFrame = selectedObject.CFrame * CFrame.Angles(math.rad(rotationStep), 0, 0)
								end
							end
						end
					end)
		...
		local cptsOnMouseClickT = nil
		Main.CreateApp({
		Name = "Click To Teleport", 
		Icon = "rbxassetid://18977106166", 
		OnClick = function(callback)
			if callback then
				local mousedas = Main.Mouse
				cptsOnMouseClickT = mousedas.Button1Down:Connect(function()
					pcall(function()
						local object = mousedas.Target
						if nodes[object] then
							game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame = object.CFrame
						end
					end)
				end)
			else if cptsOnMouseClickT ~= nil then cptsOnMouseClickT:Disconnect() cptsOnMouseClickT = nil end end
		end})

		local cptsOnMouseClickGA = nil
		Main.CreateApp({
		Name = "Click Tp To me", 
		Icon = "rbxassetid://18977142513", 
		OnClick = function(callback)
			if callback then
				local mousedsas = Main.Mouse
				cptsOnMouseClickT = mousedsas.Button1Down:Connect(function()
					pcall(function()
						local object = mousedsas.Target
						if nodes[object] then
							object.CFrame = game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame
						end
					end)
				end)
			else if cptsOnMouseClickGA ~= nil then cptsOnMouseClickGA:Disconnect() cptsOnMouseClickGA = nil end end
		end})

		Main.CreateApp({
		Name = "Destroy Dex", 
		Icon = "rbxassetid://18977091845", 
		OnClick = function(callback)
		if callback then
			for _, DexUi in ipairs(game.CoreGui.HiddenUI:GetChildren()) do
			DexUi:Destroy()
			end
		end
	end})
	...
	Main.Init = function()
		...
		Main.MiscIcons:SetDict({
			Reference = 0,             Cut = 1,                     Cut_Disabled = 2,        Copy = 3,               Copy_Disabled = 4,    
			Paste = 5,                 Paste_Disabled = 6,          Delete = 7,              Delete_Disabled = 8,    Group = 9,             
			Group_Disabled = 10,       Ungroup = 11,                Ungroup_Disabled = 12,   TeleportTo = 13,        Rename = 14,           
			JumpToParent = 16,         ExploreData = 16,            Save = 17,               CallRemotes = 18,       CallRemote = 19,       
			Undo = 20,                 Undo_Disabled = 21,          Redo = 22,               Redo_Disabled = 23,     Expand_Over = 24,      
			Expand = 25,               Collapse_Over = 26,          Collapse = 27,           SelectChildren = 28,    SelectChildren_Disabled = 29,    
			InsertObject = 30,         ViewScript = 31,             AddStar = 32,            RemoveStar = 33,        Script_Disabled = 34,  
			LocalScript_Disabled = 35, Play = 36,                   Pause = 37,              Rename_Disabled = 38,   TeleportToObj = 13,
			DeletedTextures = 7,
		})
		...
		intro.SetProgress("Fetching Roblox Version And Copied SVT",0.2)
		if Main.Elevated then
		...
		intro.SetProgress("Optimized Version",0.5)
		Lib.FastWait(2.1)

		intro.SetProgress("checkcaller, Fix Errors",0.65)
		Lib.FastWait(4)
		...
		intro.SetProgress("Load Call, Pcall, RemoteF",0.95)
		Lib.FastWait(1.25)
		-- Done
